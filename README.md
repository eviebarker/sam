...
- Med reminders speak via Piper every 5 minutes during the 30-minute window using their `speak_text`, with a **randomized prefix** (e.g., “Hey Sam,”, “Sam,”, “Quick reminder, Sam:”).
- Non‑med reminders speak once (first fire only) and also use the randomized prefix pool.
...
- Mixed scheduling is only used when **multiple items** are detected; single items follow the normal event/task/reminder paths.
